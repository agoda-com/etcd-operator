...
func TestCluster(t *testing.T) {
	...
	t.Run("scale", func(t *testing.T) {
		scale(t, ctx, kcl, cluster, 5)
		poll(t, ctx, kcl, cluster, 2*time.Minute, available)

		t.Logf("cluster %q scaled up to 5 members", cluster.Name)

		scale(t, ctx, kcl, cluster, 3)
		poll(t, ctx, kcl, cluster, time.Minute, available)
	...
	t.Run("evict pod", func(t *testing.T) {
		...
		poll(t, ctx, kcl, cluster, 2*time.Minute, func(cluster *apiv1.EtcdCluster) bool {
	...
	t.Run("upgrade", func(t *testing.T) {
		...
		poll(t, ctx, kcl, cluster, 3*time.Minute, func(cluster *apiv1.EtcdCluster) bool {
