...
func etcdClient(t testing.TB, ctx context.Context, kcl client.Client, cluster *apiv1.EtcdCluster) *etcdv3.Client {
	t.Helper()
	...
	tlsConfig, err := etcd.TLSConfig(etcd.LoadSecret(ctx, kcl, key))
	if err != nil {
		t.Fatal("load credentials:", err)
	}

	cl, err := etcd.Connect(ctx, tlsConfig, cluster.Status.Endpoint)
	if err != nil {
...
func createCluster(t testing.TB, ctx context.Context, kcl client.Client, timeout time.Duration, spec apiv1.EtcdClusterSpec) *apiv1.EtcdCluster {
	t.Helper()
	...
	poll(t, ctx, kcl, cluster, timeout, available)
...
func poll[T client.Object](t testing.TB, ctx context.Context, kcl client.Client, obj T, timeout time.Duration, f func(obj T) bool) {
	t.Helper()
...
func scale(t testing.TB, ctx context.Context, kcl client.Client, obj client.Object, replicas int32) {
	scale := &autoscalingv1.Scale{}
	err := kcl.SubResource("scale").Get(ctx, obj, scale)
	if err != nil {
		t.Fatal("get scale:", err)
	}

	scale.Spec.Replicas = replicas
	err = kcl.SubResource("scale").Update(ctx, obj, client.WithSubResourceBody(scale))
	if err != nil {
		t.Fatal("update scale:", err)
	}
}

func triggerCronJob(t testing.TB, ctx context.Context, kcl client.Client, key client.ObjectKey, timeout time.Duration) {
	t.Helper()
